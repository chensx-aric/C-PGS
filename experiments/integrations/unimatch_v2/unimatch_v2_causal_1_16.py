...
# CUDA_VISIBLE_DEVICES=0 python unimatch_v2_causal1.py --config ./configs/AC.yaml --labeled-id-path ./data --unlabeled-id-path ./data --save-path ./result5
...
def main():
    ...
    trainset_u = SemiDatasetAdapted(
        cfg['dataset'], 'train_u', args.data,cfg['crop_size']
    )
    trainset_l = SemiDatasetAdapted(
        cfg['dataset'], 'train_l', args.data, cfg['crop_size'], nsample=len(trainset_u.ids)
    )
