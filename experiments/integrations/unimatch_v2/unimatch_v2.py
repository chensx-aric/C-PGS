...
from unimatch_v2_causal import predict_single_image, cfg1
...
def main():
    ...
    trainset_l = SemiDatasetAdapted(
        cfg['dataset'], 'labeled_5_new', cfg['crop_size']
    )
    trainset_u = SemiDatasetAdapted(
        cfg['dataset'], 'train_new', cfg['crop_size']
    )
    valset = SemiDatasetAdapted(
        cfg['dataset'], 'test_new', cfg['crop_size']
    )
    ...
    # pred_mask, color_mask, overlay = predict_single_image(model, '500177.jpg', cfg1)
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, ((img_x, mask_x),
                (img_u_w, img_u_s1, img_u_s2, ignore_mask, instance_mask, _, cutmix_box1, cutmix_box2, cls_prior)) in enumerate(loader):
