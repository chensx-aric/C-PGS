...
from dataset.semi import SemiDataset, SemiDatasetAdapted1
...
# CUDA_VISIBLE_DEVICES=2 python unimatch_v2_causal_1_2_new.py --config ./configs/AC.yaml --labeled-id-path ./data --unlabeled-id-path ./data --save-path ./1_2_New --data 0.5
...
def refine_with_sam(pred_mask, sam_instances=None, id=0):
    ...
    save_fused_only(fused_mask, f"{id}.png")
    return fused_mask
...
class PPPFusion(nn.Module):
    ...
    def forward(self, p_model, p_prior):
        x = torch.cat([p_model, p_prior], dim=1)  # [B, 2C, H, W]
        logits_fused = self.conv(x)  # [B, C, H, W]
        return torch.softmax(logits_fused, dim=1)

def compute_ppp_loss_with_prior(p_model, p_prior, ppp_out, epoch, eps=1e-8):

    # ---------------------------
    # 1. Posterior consistency
    # KL(ppp_out || p_model)
    # ---------------------------
    L_post = (ppp_out.clamp(min=eps) *
              (ppp_out.clamp(min=eps).log() - p_model.clamp(min=eps).log())
             ).sum(dim=1).mean()

    # ---------------------------
    # 2. Prior filtering
    # ---------------------------
    L_prior = (ppp_out * (1.0 - p_prior)).mean()

    # ---------------------------
    # 3. Weight schedule
    # ---------------------------
    alpha = torch.exp(torch.tensor(-epoch / 40.0, device=p_model.device))
    beta = 1.0 - alpha

    return beta * L_post + alpha * L_prior
...
def main():
    ...
    nsample_fixed = 3000
    trainset_u = SemiDatasetAdapted1(
        cfg['dataset'], 'train_u', args.data,cfg['crop_size'], nsample=nsample_fixed
    )
    trainset_l = SemiDatasetAdapted1(
        cfg['dataset'], 'train_l', args.data, cfg['crop_size'], nsample=nsample_fixed
    )
    valset = SemiDatasetAdapted1(
    ...
    model.load_state_dict(checkpoint['model'])
    # label_mask = np.load('./300131.npy').astype(np.uint8)
    # pred_mask, color_mask, overlay = predict_single_image(model, '300131.jpg', label_mask, cfg1)
    mIoU, iou_class = evaluate(model, valloader, 'original', cfg, multiplier=14, measure_time=True, warmup=5)
    ...
    #         #     ignore_prior[(pixel_allowed == 0) & (conf_u_w < 0.5)] = 255
    #         # else:
    #         #     ignore_prior = torch.zeros_like(mask_u_w)
    #         #     ignore_prior[(pixel_allowed == 0) & (conf_u_w < 0.2)] = 255
    ...
    #         # )
    #         # with torch.no_grad():
    #         #     pred_u_w = model_ema(img_u_w)   # [B,C,H,W]

    #         #     # ---- class prior to float & device ----
    #         #     cls_prior_f = cls_prior.float().to(pred_u_w.device)   # [B,C]

    #         #     # ---- prior strength decay ----
    #         #     prior_alpha_init = 0.6
    #         #     prior_alpha_min  = 0.01
    #         #     decay_epochs = 80

    #         #     prior_alpha = max(
    #         #         prior_alpha_min,
    #         #         prior_alpha_init * (1 - epoch / decay_epochs)
    #         #     )

    #         #     # ---- class-aware logit bias ----
    #         #     # prior=1 → 0 bias
    #         #     # prior=0 → -prior_alpha
    #         #     prior_bias = (cls_prior_f.unsqueeze(-1).unsqueeze(-1) - 1) * prior_alpha

    #         #     pred_u_w = pred_u_w + prior_bias
    #         #     p_model = torch.softmax(pred_u_w, dim=1)

    #         #     conf_u_w, mask_u_w = p_model.max(dim=1)
    ...
    #     print(f"Peak training GPU memory: {peak_memory:.2f} GB")
